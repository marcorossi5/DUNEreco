...
def image_plots(dirname, threshold):
    ...
    legend_target = [Line2D([0], [0], marker='o', color='white', label='No Hit',
                          markerfacecolor='white', markeredgecolor='black', markersize=7),
    ...
    ax.set_xlabel("Time", fontsize=16)
    z = ax.imshow((roi[1][480:]>0.5).astype(int) - clear[480:], cmap=cmap, aspect='auto')
    ax.tick_params(axis='x', which='both', direction='in',
                   top=True, labeltop=False,
                   bottom=True, labelbottom=True)
    ax.tick_params(axis='y', which='both', direction='in',
    ...
    suptitle = f'ProtoDUNE-SP Simulation Preliminary\nMismatched points, $t= {int(threshold)}$'
