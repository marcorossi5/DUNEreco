...
def fit_and_plot_metrics(dn, clear):
    ...
    use = [x[1] for x in Dsort]
    err = [x[2] for x in Dsort]
    bar_plot(lang, use, err, fname, r'{E}[$|$Target-Output$|$] on integrated ADC counts')
...
def image_plots(dirname):
    ...
    fig = plt.figure(figsize=(12, 9))
    fig.suptitle('ProtoDUNE-SP Simulation Preliminary', fontsize=20,
                 y=0.95)
