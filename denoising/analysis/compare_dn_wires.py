...
def image_arrays(dirname):
    ...
    pandora_wires = np.load(fname)[0,0]
...
def image_plots(dirname):
    ...
    dir_name = 'denoising/benchmarks/plots/'
    fname = dir_name + 'dn_pandora_wires.pdf'
    ...
    ax.plot(dn[2][500], lw=0.3, label='Pandora', color='forestgreen')
    ...
    ax.plot(dn[2][0], lw=0.3, label='Pandora', color='forestgreen')
    ax.set_xlim([0,6000])
    ax.set_ylim([-1.5,1.5])
