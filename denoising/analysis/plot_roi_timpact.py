...
def main():
    ...
    ax.fill_between(thresholds, sns_mean[:,0]-sns_std[:,0],
                    sns_mean[:,0]+sns_std[:,0], alpha=0.5,
                    step='post', color="lime")
    ax.step(thresholds, sns_mean[:,1], color="coral", label='gcnn', lw=0.5, where='post')
    ax.fill_between(thresholds, sns_mean[:,1]-sns_std[:,1],
                    sns_mean[:,1]+sns_std[:,1], alpha=0.5,
    ...
    ax.axhline(.975, lw=0.3, linestyle='--', color='lightslategrey')
    ax.grid(lw=0.3, alpha=0.2, color='lightslategrey')
    ax.set_xlabel('Threshold [ADC]')
    ax.legend(frameon=False, ncol=2)
    plt.savefig('denoising/benchmarks/plots/roi_timpact.pdf',
                bbox='tight',dpi=300)
    plt.close()
